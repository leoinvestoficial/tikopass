...
import { Send, Clock, CheckCircle2, XCircle, MessageSquare, Loader2, CreditCard, ShieldCheck, ArrowLeftRight, Trophy, Zap, TrendingUp, User } from "lucide-react";
      ...
      <div className="flex-1">
        ...
        <div className="border-b border-border bg-card">
          <div className="container py-8">
            <h1 className="text-3xl font-display font-bold text-foreground">Negociações</h1>
            <p className="text-muted-foreground mt-1 text-sm">Acompanhe todas as suas transações</p>
            ...
            {totalNeg > 0 && (
              <div className="grid grid-cols-3 gap-4 mt-6 max-w-lg">
                <div className="bg-background rounded-xl p-4 border border-border text-center">
                  ...
                  <p className="text-2xl font-bold text-foreground">{totalNeg}</p>
                </div>
                <div className="bg-background rounded-xl p-4 border border-border text-center">
                ...
                <div className="bg-background rounded-xl p-4 border border-border text-center">
                  <div className="flex items-center justify-center gap-1.5 text-success mb-1">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-wider">Fechadas</span>
                  </div>
        ...
        <div className="container py-8">
          {loading ? (
            ...
            <div className="text-center py-20 space-y-4">
              ...
              <h3 className="font-display font-bold text-xl">Nenhuma negociação ainda</h3>
              <p className="text-sm text-muted-foreground max-w-xs mx-auto">
                Quando você fizer uma oferta ou receber uma, as negociações aparecerão aqui.
              </p>
              <Button onClick={() => navigate("/")} className="rounded-xl gap-2 mt-4">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              ...
              {/* Chat panel */}
              <div className="lg:col-span-8 bg-card border border-border rounded-2xl overflow-hidden flex flex-col min-h-[500px] shadow-sm">
                {activeNeg ? (
                  <>
                    {/* Chat header */}
                      ...
                      <div className="flex items-start justify-between flex-wrap gap-3">
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          {activeNeg.status === "pending" && isSeller && (
                            <>
                              <Button size="sm" onClick={() => handleStatusUpdate(activeNeg.id, "accepted")} className="rounded-xl text-xs gap-1.5 bg-success hover:bg-success/90 text-white">
                                <CheckCircle2 className="w-3.5 h-3.5" /> Aceitar
                              </Button>
                              <Button size="sm" variant="outline" onClick={() => { setCounterPrice(String(activeNeg.offer_price)); setCounterDialog(true); }} className="rounded-xl text-xs gap-1.5">
                                <ArrowLeftRight className="w-3.5 h-3.5" /> Contra
                              </Button>
                              <Button size="sm" variant="ghost" onClick={() => handleStatusUpdate(activeNeg.id, "rejected")} className="rounded-xl text-xs text-destructive hover:text-destructive hover:bg-destructive/10">
                                Recusar
                              </Button>
                            </>
                          )}
                          {activeNeg.status === "counter" && isBuyer && (
                            <>
                              <Button size="sm" onClick={() => handleAcceptCounter(activeNeg.id)} className="rounded-xl text-xs gap-1.5 bg-success hover:bg-success/90 text-white">
                                <CheckCircle2 className="w-3.5 h-3.5" /> Aceitar R$ {activeNeg.counter_offer_price?.toLocaleString("pt-BR")}
                              </Button>
                              <Button size="sm" variant="ghost" onClick={() => handleStatusUpdate(activeNeg.id, "rejected")} className="rounded-xl text-xs text-destructive hover:text-destructive hover:bg-destructive/10">
                                Recusar
                              </Button>
                          ...
                          {activeNeg.status === "accepted" && isBuyer && activeNeg.payment_status !== "paid" && (
                            <Button
                              size="sm"
                              className="rounded-xl text-xs gap-1.5 animate-pulse"
                              onClick={async () => {
                                try {
                          ...
                          {activeNeg.payment_status === "paid" && (
                            <Badge className="bg-success/10 text-success border-success/20 text-xs gap-1 px-3 py-1.5">
                    ...
                    {/* Messages */}
                    <div className="flex-1 p-5 space-y-4 overflow-y-auto bg-gradient-to-b from-background/50 to-background">
                      {messages.length === 0 && (
                        <div className="text-center py-12 space-y-2">
                          <div className="w-12 h-12 rounded-2xl bg-muted flex items-center justify-center mx-auto">
                            <MessageSquare className="w-6 h-6 text-muted-foreground" />
                          </div>
                          <p className="text-sm text-muted-foreground">Nenhuma mensagem. Inicie a conversa!</p>
                        </div>
                      )}
                    ...
                    <div className="p-4 border-t border-border bg-card">
                      <div className="flex gap-2">
                        ...
                        />
                        <Button size="icon" className="rounded-xl shrink-0 h-11 w-11" onClick={handleSend} disabled={sending || !message.trim()}>
